...
function render_me() {
    ...
    function check_if_connected() {
        web3.eth.getAccounts().then(async (tx) => {
            if (tx[0] !== undefined) {
                $('#mint_btn').html("Mint")
                $('#mint_link').html(truncateString(String(tx[0]), 10));
    ...
    function trigger_minting(amt) {
        if (window.ethereum.networkVersion == chainId) {
            const contract = new web3.eth.Contract(t_abi, t_address);
            web3.eth.getAccounts().then(async (tx) => {
                amt = (amt * Math.pow(10, 18)).toFixedSpecial(0)
                await contract.methods.mint(String(amt)).send({ from: tx[0], value: amt, gasLimit: "28500" }).then(function (res, err) {
                    if (res) {
